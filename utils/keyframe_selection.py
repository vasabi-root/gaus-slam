...
def get_pointcloud(depth, intrinsics, w2c, sampled_indices):
    ...
    depth_z = depth[sampled_indices[:, 0], sampled_indices[:, 1], 0]
...
def keyframe_selection_overlap(cameras, time_idx, k, pixels=1600):
        """
        ...
        """
        curr_camera = cameras[time_idx]
        gt_depth = curr_camera.gt_depth
        intrinsics = curr_camera.intrinsics
        w2c = curr_camera.get_w2c
        
        # Radomly Sample Pixel Indices from valid depth pixels
        width, height = gt_depth.shape[1], gt_depth.shape[0]
        ...
        list_keyframe = []
        for keyframeid, camera in enumerate(cameras):
            # Get the estimated world2cam of the keyframe
            est_w2c = camera.get_w2c
