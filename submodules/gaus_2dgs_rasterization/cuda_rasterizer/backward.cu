...
renderCUDA(
	...
	const float* __restrict__ depth_std,
	bool use_sa)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float conf = 1;
			if (use_sa) {
				// mstd = mstd - w * (c_d - mm) * (c_d - mm);
				conf = T < 0.5 ? exp(-(c_d - mm) * (c_d - mm) / (4 * max(mstd / (1 - T_final), 1e-7))) : 1.f;
...
__device__ void computeCamMaskFromSH(int deg, int W, int H, float2 pixf, const float* camsh, const bool clamped, const float dL_dout_mask, float* dL_dcamsh)
{
	// Compute intermediate values, as it is done during forward
	float f = max(W, H) / 2;
	...
	// Use PyTorch rule for clamping: if clamping was applied,
	// gradient becomes 0.
	float dL_dmask = dL_dout_mask;
	dL_dmask *= clamped ? 0 : 1;
	...
	// Target location for this Gaussian to write SH gradients to
	float* dL_dsh = dL_dcamsh;

	// No tricks here, just high school-level calculus.
	float dRGBdsh0 = SH_C0;
	dL_dsh[0] += dRGBdsh0 * dL_dmask;
	if (deg > 0)
	{
		float dRGBdsh1 = -SH_C1 * y;
		float dRGBdsh2 = SH_C1 * z;
		float dRGBdsh3 = -SH_C1 * x;
		dL_dsh[1] += dRGBdsh1 * dL_dmask;
		dL_dsh[2] += dRGBdsh2 * dL_dmask;
		dL_dsh[3] += dRGBdsh3 * dL_dmask;
		...
		if (deg > 1)
		{
			...
			float dRGBdsh7 = SH_C2[3] * xz;
			float dRGBdsh8 = SH_C2[4] * (xx - yy);
			dL_dsh[4] += dRGBdsh4 * dL_dmask;
			dL_dsh[5] += dRGBdsh5 * dL_dmask;
			dL_dsh[6] += dRGBdsh6 * dL_dmask;
			dL_dsh[7] += dRGBdsh7 * dL_dmask;
			dL_dsh[8] += dRGBdsh8 * dL_dmask;
			...
			if (deg > 2)
			{
				...
				float dRGBdsh14 = SH_C3[5] * z * (xx - yy);
				float dRGBdsh15 = SH_C3[6] * x * (xx - 3.f * yy);
				dL_dsh[9] += dRGBdsh9 * dL_dmask;
				dL_dsh[10] += dRGBdsh10 * dL_dmask;
				dL_dsh[11] += dRGBdsh11 * dL_dmask;
				dL_dsh[12] += dRGBdsh12 * dL_dmask;
				dL_dsh[13] += dRGBdsh13 * dL_dmask;
				dL_dsh[14] += dRGBdsh14 * dL_dmask;
				dL_dsh[15] += dRGBdsh15 * dL_dmask;
			}
		}
	}
}
...
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y) 
render_camshCUDA(
	const float* __restrict__ camsh, 
	const float* __restrict__ dL_dout_mask,
	int D, int W, int H,

	bool* __restrict__ camclamp,
	float* __restrict__ dL_dcamsh
) {
	auto block = cg::this_thread_block();
	...
	if (!inside) return;
	computeCamMaskFromSH(D, W, H, pixf, camsh, camclamp[pix_id], dL_dout_mask[pix_id], dL_dcamsh);
}
